...
\begin{enumerate}
  ...
  \item Extraer los nombres de las clases de Python y UML, para poder comprobar cuales de ellas
  están representadas.
  \item Optimizar código para los proyectos disponibles, tras realizar pruebas con ellos.
...
FIXME: yo añadiría fechas.

\begin{enumerate}
  \item Reunión con el tutor dónde me explica su idea del tema del proyecto y el objetivo que se persigue.
  \item Estudiar y entender el lenguaje de modelado UML, para qué se usa, cómo es, dónde se usa...
...
\begin{figure}[htb]
  ...
  \caption{Representación de clase. FIXME: no se referencia ni se explica esta figura en el texto principal}
...
Para el caso de el Listado~\ref{lst:ejemploxmi}, se observa cómo se escriben estos diagramas. El código se ha obtenido a partir de uno de los proyectos analizados, y se ha simplificado, de manera que sea más fácil observar los elementos deseados:
...
FIXME: Explicar para qué se ha utilizado Django en el proyecto
...
Normalmente es utilizada para la localización de las definiciones o tipos, por numerosos editores de código o entornos de desarrollo integrados (IDEs por sus siglas in inglés) y otras utilidades.
...
Por ejemplo, el resultado de análisis de un fichero con una clase llamada \textit{claseprueba} sería el siguiente:
  ...
  FIXME: el ejemplo debería ser más ampli. No sólo una línea, que sean cuatro o cinco.

Simplemente por el identificador del final, c, ya sabemos que es una clase, y el nombre lo podemos extraer del principio de la línea.

FIXME: Explicar para qué se ha utilizado ctags en el proyecto
...
FIXME: Explicar para qué se ha utilizado Python en el proyecto


\subsection{API de GitHub}
\label{subsec:apigithub}
\textbf{GitHub}\footnote{https://github.com} es una plataforma para almacenar repositorios, utilizando el control de versiones de Git (ver seccion~\ref{subsec:git}), en la que destaca el desarrollo colaborativo. Ofrece, además de los repositorios git, funcionalidades como un sistema de gestión de erratas, wikis, una web pública y la posibilidad de que hacer ramificaciones (\emph{forks}) de un repositorio.
...
FIXME: Contar que se ha utilizado GitHub también para alojar nuestra herramienta y en otro repositorio la memoria
...
FIXME: Explicar para qué se ha utilizado git en el proyecto
...
FIXME: Explicar para qué se ha utilizado HTML5 en el proyecto
...
FIXME: Explicar para qué se ha utilizado CSS en el proyecto
...
FIXME: Explicar para qué se ha utilizado JavaScript en el proyecto
...
FIXME: Explicar para qué se ha utilizado Bootstrap en el proyecto
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    a la que queremos asoViar, en este caso \textit{results}, sera asociada a la vista \emph{results} ubicada en views.results.
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    Todas las clases de todos los archivos .py serán añadidas a un archivo.
...
Para no perder los datos se van almacenando en un fichero .csv. FIXME: explicar qué es eso de un CSV
...
podemos ver a groso modo, que la mayoría de clases Python no está representada en los diagramas UML, sin tener en cuenta
